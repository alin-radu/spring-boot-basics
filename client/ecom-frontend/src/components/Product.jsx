...
const Product = () => {
  console.log('%c-> developmentConsole: Product= ', 'color:#77dcfd');
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        console.log('%c-> developmentConsole: id= ', 'color:#77dcfd', id);
    ...
    const fetchImage = async () => {
      const response = await axios.get(`http://localhost:8080/api/products/${id}/image`, {
        responseType: 'blob',
      });
